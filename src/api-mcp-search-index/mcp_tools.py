...
class ToolProperty:
    """Helper class for defining MCP tool properties in the correct Azure Funct            logger.error(f"Error in hybrid_search_mcp: {e}")
            return f"❌ Error performing hybrid search: {str(e)}"ns format."""
...
hybrid_search_properties = [
    ToolProperty("query", "string", "Search query text"),
    ToolProperty("top_k", "integer", "Number of results to return (default: 5, max: 50)"),
    ToolProperty("document_types", "string", "OPTIONAL: Filter by specific document types as comma-separated list (e.g., 'LABELS,MANUALS'). Leave empty to search ALL documents. Use get_document_types tool to see available types if filtering is needed."),
    ToolProperty("include_content", "boolean", "Include full content in results (default: true)"),
    ToolProperty("vector_weight", "number", "Weight for vector similarity (default: 0.6, range: 0.0-1.0)"),
    ToolProperty("keyword_weight", "number", "Weight for keyword search (default: 0.4, range: 0.0-1.0)")
]

semantic_search_properties = [
    ToolProperty("query", "string", "Search query text"),
    ToolProperty("top_k", "integer", "Number of results to return (default: 3, max: 50)"),
    ToolProperty("document_types", "string", "OPTIONAL: Filter by specific document types as comma-separated list (e.g., 'LABELS,MANUALS'). Leave empty to search ALL documents. Use get_document_types tool to see available types if filtering is needed."),
    ToolProperty("include_content", "boolean", "Include full content in results (default: true)")
]
...
document_info_properties = [
    ToolProperty("document_id", "string", "Optional document ID to get specific document details")
]

list_documents_properties = [
    ToolProperty("document_type", "string", "Filter by document type (use get_document_types tool to see available types)"),
    ToolProperty("limit", "integer", "Maximum number of documents to return (default: 20, max: 100)")
]
...
def register_mcp_tools(app: func.FunctionApp):
    ...
    @app.generic_trigger(
        ...
        description="Perform semantic search across ALL documents using vector similarity for conceptual matching. BEST FOR: 'how to' questions, procedural queries, conceptual relationships, troubleshooting guidance. SEARCHES ALL DOCUMENT TYPES by default - only specify document_types parameter if you need to filter to specific types. PERFORMANCE: <6000ms first query, <2000ms subsequent. Results >0.3 similarity are highly relevant.",
    ...
    @app.generic_trigger(
        ...
        description="Perform fast keyword search across ALL documents using full-text search with BM25 scoring for exact term matching. BEST FOR: exact terms, product names, technical specifications, ID numbers, specific phrases, regulatory data (LD50, EPA numbers). SEARCHES ALL DOCUMENT TYPES by default - only specify document_types parameter if you need to filter to specific types. PERFORMANCE: <1000ms response time.",
    ...
    @app.generic_trigger(
        ...
        description="Get detailed information about document types, counts, or specific document details. Use for document inventory, processing status verification, or when user asks about specific documents. Provides comprehensive metadata including chunk counts, processing status, and file details.",
...
• **Blob Name:** {metadata.blob_name}"""
    ...
    @app.generic_trigger(
        ...
        description="List recent documents with optional filtering by document type. Use when user needs document inventory ('what documents are available'), processing status verification, or document metadata overview. Shows document processing status, chunk counts, and file information. NOTE: For general inventory questions ('what's available'), first call get_document_types to see all types, then optionally call this tool without document_type filter or try get_document_info without document_id.",
    ...
    @app.generic_trigger(
        ...
        description="Perform hybrid search across ALL documents using Reciprocal Rank Fusion (RRF) combining vector similarity with keyword matching for optimal ranking. BEST FOR: Most complex queries, mixed conceptual/technical questions, comprehensive research needs. RECOMMENDED DEFAULT for general queries. SEARCHES ALL DOCUMENT TYPES by default - only specify document_types parameter if you need to filter to specific types. PERFORMANCE: <3500ms with superior result quality.",
